...
{
	// TODO
	// d_Result: ACCUM_N * sizeof(float) = BLOCK_N * THREAD_N * sizeof(float)
	// d_Input: N * sizeof(float) = 32 * 1024 * 1024 / GPU_N

	int i  = threadIdx.x + blockDim.x * blockIdx.x;
	int threadN = gridDim.x * blockDim.x;

	float sum = 0;
	for(int pos = i; pos < N; pos += threadN) {
		sum += d_Input[pos];
	}
...
int main(int argc, char **argv)
{
	...
	const int  ACCUM_N = BLOCK_N * THREAD_N; // 32 * 256

	printf("Starting simpleMultiGPU\n");
	checkCudaErrors(cudaGetDeviceCount(&GPU_N)); // get a device count
	...
	for (i = 0; i < GPU_N; i++)
	{
		plan[i].dataN = DATA_N / GPU_N; // per GPU data size
	}

	//Take into account "odd" data sizes
	for (i = 0; i < DATA_N % GPU_N; i++)
	{
		plan[i].dataN++; // because of odd data size
	}
	...
	for (i = 0; i < GPU_N; i++)
	{
		// TODO: setup device & create dedicated streams for each of them
		cudaSetDevice(i);
		...
		// initialize host-side h_Data[] with randomized data
	...
	for (i = 0; i < GPU_N; i++)
	{
		...
		// TODO: Perform GPU computations
		reduceKernel<<<BLOCK_N, THREAD_N, 0, plan[i].stream>>>(plan[i].d_Sum, plan[i].d_Data, plan[i].dataN);
		getLastCudaError("reduceKernel() execution failed.\n");

		// TODO: Read back GPU results
